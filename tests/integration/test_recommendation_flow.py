...
from app.models import User, Book, Author, ReadingEvent
...
def test_full_recommendation_flow(client_with_queue, db_session):
    ...
    events = [
        ...
        ReadingEvent(user_id=target_user.id, book_id=book1.id, rating=5.0, completed_at=datetime.utcnow()),
        ReadingEvent(user_id=target_user.id, book_id=book2.id, rating=4.0, completed_at=datetime.utcnow()),
        ReadingEvent(user_id=target_user.id, book_id=book3.id, rating=3.0, completed_at=datetime.utcnow()),  # ← НОВАЯ
        
        # Похожий пользователь
        ReadingEvent(user_id=similar_user.id, book_id=book1.id, rating=5.0, completed_at=datetime.utcnow()),
        ReadingEvent(user_id=similar_user.id, book_id=book2.id, rating=5.0, completed_at=datetime.utcnow()),
        ReadingEvent(user_id=similar_user.id, book_id=book4.id, rating=4.5, completed_at=datetime.utcnow()),  # ← кандидат для рекомендации
    ]
    ...
    assert book4_id in recommended_ids, f"Book {book4_id} should be recommended, got: {recommended_ids}"
