...
from app.models import ReadingEvent, Book, RecommendationCache
...
class RecommendationService:
    ...
    def __init__(self, db: Session, engine: Optional[RecommenderEngine] = None):
        self.db = db
    ...
    def generate_for_user(self, user_id: int) -> UserRecommendations:
        ...
        return self.engine.predict_for_user(
            user_id=user_id,
            events=events,
            all_book_ids=all_books
        )
    ...
    def save_to_cache(self, result: UserRecommendations) -> RecommendationCache:
        ...
        existing = self.db.query(RecommendationCache).filter(
            RecommendationCache.user_id == result.user_id
        ).first()
    ...
    def get_from_cache(self, user_id: int) -> Optional[UserRecommendations]:
        ...
        recommendations = [
            ...
            for bid, scr in zip(book_ids, scores)
        ]
